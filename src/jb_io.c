...
#elif PORT==EMBEDDED
#define GPIO_PORT_TDO                 "5"
#define GPIO_PORT_TDI                 "6"
#define GPIO_PORT_TMS                 "12"
#define GPIO_PORT_TCK                 "13"
#define GPIO_PORT_INUSE               "16"
#define DRIVER_GPIO_EXPORT            "/sys/class/gpio/export"
#define DRIVER_GPIO_UNEXPORT          "/sys/class/gpio/unexport"
#define DRIVER_GPIO_TDO_DIRECTION     "/sys/class/gpio/gpio5/direction"
#define DRIVER_GPIO_TDI_DIRECTION     "/sys/class/gpio/gpio6/direction"
#define DRIVER_GPIO_TMS_DIRECTION     "/sys/class/gpio/gpio12/direction"
#define DRIVER_GPIO_TCK_DIRECTION     "/sys/class/gpio/gpio13/direction"
#define DRIVER_GPIO_INUSE_DIRECTION   "/sys/class/gpio/gpio16/direction"
#define DRIVER_GPIO_TDO_VALUE         "/sys/class/gpio/gpio5/value"
#define DRIVER_GPIO_TDI_VALUE         "/sys/class/gpio/gpio6/value"
#define DRIVER_GPIO_TMS_VALUE         "/sys/class/gpio/gpio12/value"
#define DRIVER_GPIO_TCK_VALUE         "/sys/class/gpio/gpio13/value"
#define DRIVER_GPIO_INUSE_VALUE       "/sys/class/gpio/gpio16/value"
#endif
...
void InitGPIO()
{
  ...
  // GPIO Reserve
  /*-------------------------------------------------------*/
	init_ok = DriverWrite(DRIVER_GPIO_EXPORT, GPIO_PORT_TDO, sizeof(GPIO_PORT_TDO));
	init_ok += DriverWrite(DRIVER_GPIO_EXPORT, GPIO_PORT_TDI, sizeof(GPIO_PORT_TDI));
	init_ok += DriverWrite(DRIVER_GPIO_EXPORT, GPIO_PORT_TMS, sizeof(GPIO_PORT_TMS));
	init_ok += DriverWrite(DRIVER_GPIO_EXPORT, GPIO_PORT_TCK, sizeof(GPIO_PORT_TCK));
	init_ok += DriverWrite(DRIVER_GPIO_EXPORT, GPIO_PORT_INUSE, sizeof(GPIO_PORT_INUSE));
	if (init_ok != 0)
	{
		fprintf( stderr, "Error: Driver initialization fail... Exiting...\n" );
		CloseGPIO();
  ...
  /*-------------------------------------------------------*/
	usleep(100000);
	init_ok = DriverWrite(DRIVER_GPIO_TDO_DIRECTION, "in", 3);
	init_ok += DriverWrite(DRIVER_GPIO_TDI_DIRECTION, "out", 4);
	init_ok += DriverWrite(DRIVER_GPIO_TMS_DIRECTION, "out", 4);
	init_ok += DriverWrite(DRIVER_GPIO_TCK_DIRECTION, "out", 4);
	init_ok += DriverWrite(DRIVER_GPIO_INUSE_DIRECTION, "out", 4);
	if (init_ok != 0)
	{
		fprintf( stderr, "Error: Driver direction setting fail... Exiting...\n" );
		CloseGPIO();
		exit(1);
	}
  ...
  /*-------------------------------------------------------*/
	fhandle_TDO = open(DRIVER_GPIO_TDO_VALUE, O_RDONLY);
	fhandle_TDI = open(DRIVER_GPIO_TDI_VALUE, O_WRONLY);
	fhandle_TMS = open(DRIVER_GPIO_TMS_VALUE, O_WRONLY);
	fhandle_TCK = open(DRIVER_GPIO_TCK_VALUE, O_WRONLY);
	fhandle_INUSE = open(DRIVER_GPIO_INUSE_VALUE, O_WRONLY);
	if ((fhandle_TDO < 0) || (fhandle_TDI < 0) || (fhandle_TMS < 0) || (fhandle_TCK < 0) || (fhandle_INUSE < 0))
	{
		fprintf( stderr, "Error: Driver handling fail... Exiting...\n" );
		CloseGPIO();
		exit(1);
	}
	write(fhandle_INUSE, "1", 2);
}

/*---------------------------------------------------------*/
//  Close GPIO Driver function
/*---------------------------------------------------------*/
void CloseGPIO()
{
	volatile int result = 0;
	write(fhandle_INUSE, "0", 2);
	result = DriverWrite(DRIVER_GPIO_UNEXPORT, GPIO_PORT_TDO, sizeof(GPIO_PORT_TDO));
	result = DriverWrite(DRIVER_GPIO_UNEXPORT, GPIO_PORT_TDI, sizeof(GPIO_PORT_TDI));
	result = DriverWrite(DRIVER_GPIO_UNEXPORT, GPIO_PORT_TMS, sizeof(GPIO_PORT_TMS));
	result = DriverWrite(DRIVER_GPIO_UNEXPORT, GPIO_PORT_TCK, sizeof(GPIO_PORT_TCK));
	result = DriverWrite(DRIVER_GPIO_UNEXPORT, GPIO_PORT_INUSE, sizeof(GPIO_PORT_INUSE));
	close(fhandle_TDO);
	close(fhandle_TDI);
	close(fhandle_TMS);
	close(fhandle_TCK);
...
int ReadPort(int port)
{
	...
	char rdata = 0;
	lseek(fhandle_TDO, 0, SEEK_SET);
	read(fhandle_TDO, &rdata, 1);
//	if (rdata == '0') data = 0; else data = sig_port_maskbit[SIG_TDO][1];
	if (rdata == '0') data = 0x80; else data = 0x0;
...
void WritePort(int port,int data,int buffer_enable)
{
	...
	//if ((data & sig_port_maskbit[SIG_TDI][1]) == 0) write(fhandle_TDI, "0", 2); else write(fhandle_TDI, "1", 2);
	if ((data & 0x40) == 0) write(fhandle_TDI, "0", 2); else write(fhandle_TDI, "1", 2);
	//if ((data & sig_port_maskbit[SIG_TMS][1]) == 0) write(fhandle_TMS, "0", 2); else write(fhandle_TMS, "1", 2);
	if ((data & 0x2) == 0) write(fhandle_TMS, "0", 2); else write(fhandle_TMS, "1", 2);
	//if ((data & sig_port_maskbit[SIG_TCK][1]) == 0) write(fhandle_TCK, "0", 2); else write(fhandle_TCK, "1", 2);
	if ((data & 0x1) == 0) write(fhandle_TCK, "0", 2); else write(fhandle_TCK, "1", 2);
	//usleep(1);
