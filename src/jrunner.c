...
void main(int argc, char* argv[])
{
	...
	/* Introduction */
	fprintf(stdout,"\n=======================================================\n");
	fprintf(stdout," Mi-ke neko Tools :: JCONFIG Version %s\n", VERSION);
	fprintf(stdout," Copyright (c) 2015 Mi-ke neko Tools.\n");
	fprintf(stdout," Based on JTAGRunner(JRunner) Version %s\n", SRCVERSION);
	...
	for(i=0;i<4;i++)
	{
		if(GetData(Info_name[i],buffer,data))
		{
			...
			case 1:
				...
				jb_tolower((char *)&device_list[device_count-1].path);
				break;
			case 2:
				jb_strcpy(device_list[device_count-1].file,data);
				jb_tolower((char *)&device_list[device_count-1].file);
				break;
			case 3:
...
void ProcessFileInput(int finputid, int idcode)
{
	...
	fprintf(stdout,"Info: Start configuration process.  Please wait...\n");
	...
	for(i=0;i<file_size;i++)
	{
		...
		else
		{
			...
			for (j=0;j<8;j++ )
			{
				...
				//WritePort(sig_port_maskbit[SIG_TDI][0], bit, BUFFER_ON);
				//WritePort(sig_port_maskbit[SIG_TDI][0], bit | 0x01, BUFFER_ON);
				//WritePort(sig_port_maskbit[SIG_TDI][0], bit, BUFFER_ON);
			}
		}
		if ((i & 0x1fff) == 0x1fff) fprintf(stdout, "  ->%ld/%ld\n", i, file_size);
	}
#if PORT==WINDOWS_NT
...
void Help()
{
	fprintf(stderr,"Error: Invalid number of argument! \nSyntax: \"mt_jconfig <Chain Description File(.cdf)>\"\n");
	fprintf(stderr,"Example: \"mt_jconfig project1.cdf\"\n");
}
